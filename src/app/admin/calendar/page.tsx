                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    {dayWalks.slice(0, 4).map((w) => {
                      ...
                      const firstName = w.client.name.split(" ")[0];
                      const inner = (
                        <span
                          ...
                          title={`${firstName} · ${w.serviceName ?? "Walk"} · ${WALK_STATUS_LABELS[w.status] ?? w.status}${w.worker ? ` · ${w.worker.name}` : ""}`}
                        >
                          {done && <Icon name="check" className="h-3 w-3 shrink-0" />}
                          <span className="truncate">{firstName}</span>
                          {w.numDogs > 1 && <span className="opacity-70">×{w.numDogs}</span>}
                        </span>
                      );
                      return w.bookingId ? (
